...
class TextDataProvider(object):
    ...
    def __init__(self, dataset_dir, annotation_name, validation_set=None, validation_split=None, shuffle=None,
        ...
        with open(test_anno_path, 'r') as anno_file:
            ...
            #print(info)
            test_images_org = [cv2.imread(ops.join(self.__test_dataset_dir, tmp[0]), cv2.IMREAD_COLOR) for tmp in info]
            print(test_images_org[0])
            print([tmp.shape for tmp in test_images_org])
            test_images = np.array([cv2.resize(tmp, (200, 32)) for tmp in test_images_org])#设置图像的大小


            test_labels = np.array([tmp[1].strip('\n').strip('\"') for tmp in info])
            #print("test_labels=",test_labels)

            test_imagenames = np.array([ops.basename(tmp[0]) for tmp in info])
            print("test_imagenames =",test_imagenames.shape)
        ...
        with open(train_anno_path, 'r') as anno_file:
            info = np.array([tmp.split(',') for tmp in anno_file.readlines()])
            train_images_org = [cv2.imread(ops.join(self.__train_dataset_dir, tmp[0]),cv2.IMREAD_COLOR)
                                     for tmp in info]
            train_images = np.array([cv2.resize(tmp,(200,32)) for tmp in train_images_org])

            train_labels = np.array([tmp[1].strip('\n').strip('\"') for tmp in info])
