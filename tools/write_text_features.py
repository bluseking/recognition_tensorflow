...
def write_features(dataset_dir, save_dir):
    ...
    train_images = [bytes(list(np.reshape(tmp, [200 * 32 *3]))) for tmp in train_images]
    ...
    test_images = [bytes(list(np.reshape(tmp, [200 * 32 * 3]))) for tmp in test_images]
    ...
    val_images = [bytes(list(np.reshape(tmp, [200 * 32 * 3]))) for tmp in val_images]
